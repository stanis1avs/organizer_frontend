...
export default class Chat {
    ...
    init() {
      ...
      this.media_body.addEventListener("click", (e) = >{
        ...
        e.currentTarget !== e.target.parentElement ? null: (item = e.target);
    ...
    record(typeMedia) {
      ...
      mediaLoader.record(typeMedia).then((blob) = >this.send(blob)).
      catch((err) = >alert(err));
    }
    attach() {
      this.clip_body.classList.remove("clip_active");
    ...
    send(elem) {
      ...
      if (elem instanceof Blob) {
        ...
        infoMessg.body = elem; ! elem.name ? (infoMessg.name = $ {
          this.currentDate.getTime()
        }.$ {
          extension
        }) : (infoMessg.name = elem.name);
      ...
      if (infoMessg.length !== 0) { ! infoMessg.hasOwnProperty("geo") ? (infoMessg.geo = "") : null;
        infoMessg.date = $ {
          this.checkValue(this.currentDate.getDate())
        }: $ {
          this.checkValue(this.currentDate.getMonth() + 1)
        }: $ {
          this.currentDate.getFullYear()
        },
        $ {
          this.checkValue(this.currentDate.getHours())
        }: $ {
    ...
    bin(elem, id) {
      ...
      bin.addEventListener("click", () = >{
        this.request.send("deleteMessage", {
          id
        });
      });
    }
    ...
    setFavoriteState(elem, id, isFavorite, send = false) {
      const header = elem.querySelector(".message_header");
      const existingInactive = header.querySelector(".favorite_inactive");
      const existingActive = header.querySelector(".favorite_active");
      const ensureInactiveHandler = (inactiveEl) = >{
        inactiveEl.addEventListener("click", () = >{
          this.setFavoriteState(elem, id, true, true);
        });
      };
      if (isFavorite) {
        ...
        favoriteActive.classList.add("message_header_img", "favorite_active");
        if (existingInactive) {
          existingInactive.replaceWith(favoriteActive);
        } else {
          header.append(favoriteActive);
        }
        favoriteActive.addEventListener("click", () = >{
          const inactiveEl = document.createElement("div");
          inactiveEl.classList.add("message_header_img", "favorite_inactive");
          favoriteActive.replaceWith(inactiveEl);
          ensureInactiveHandler(inactiveEl);
          this.removeFromFavoritesList(elem);
          if (send) this.request.send("favoriteDelete", id);
          this.updateMediaCounts();
        });
        if (!this.media_body_favorites.querySelector([data - id = "${elem.dataset.id}"])) {
        ...
        if (send) this.request.send("favoriteAppend", id);
        this.updateMediaCounts();
      } else {
        if (existingActive) {
          const inactiveEl = document.createElement("div");
    ...
    removeFromFavoritesList(elem) {
      ...
      const node = this.media_body_favorites.querySelector([data - id = "${id}"]);
      if (node) node.remove();
    }
    /* ----------------------- Отображение -------- ---------------------- */
    geo() {
    ...
    deleteMessage(id) {
      const node = this.messages.querySelector([data - id = "${id}"]);
      if (node) node.remove();
    ...
    renderMessages(data, favorites, position, pinnedId) {
      const flag = true;
      for (const message of data) {
        const messageElement = this.showMessage(message.id, message.message, message.date, message.geo, message.type, flag);
        if (favorites.indexOf(message.id) !== -1) {
          this.setFavoriteState(messageElement, message.id, true, false);
        }
        if (pinnedId && message.id.toString() === pinnedId) {
    ...
    updateMediaCounts() {
      const counters = {
        ...
        audios: 0,
        files: 0,
        favorites: this.media_body_favorites.childElementCount,
      };
      this.messages.querySelectorAll(".message").forEach((msg) = >{
      ...
      Object.entries(counters).forEach(([key, val]) = >{
        const li = this.media_body.querySelector([data - item = "${key}"] span);
        if (li) li.textContent = val;
      });
    }
